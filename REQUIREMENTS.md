...
При добавлении изображения в библиотеку:
1. Приложение анализирует изображение
2. Если обнаружен однородный фон — предлагает удалить
3. Action Sheet: "Remove background?" [Remove] [Keep Original]
4. Обработка занимает 1-3 секунды, показывается индикатор

Добавить можно:
1. Как файл (уже есть)
2. Из фото библиотеки (уже есть)
3. При помощи камеры (новая фича). При этом можно "кропнуть" нужную часть сфотографированного изображения.

Работает лучше всего для:
- Подписи на белой бумаге
- Штампы на светлом фоне
- Печати
